...
int main(int argc, char **argv)
{
  ...
  // User sigaction to set the new signal handler
  ...
  // 
	...
	if(chunk_size>max_chunk_size){
		...
		if(lcpid==0){
			...
			const char* path = "./part3_partioner.out";
			char *const argv[] = {
				(char *)"./part3_partioner.out",
		...
		if(search_pid == 0){
			...
			const char* exe_path = "./part3_searcher.out";
			char *const exe_argv[] = {
				(char *)"./part3_searcher.out",
				(char *)file_to_search_in,
				(char *)pattern_to_search_for,
				(char *)argv[3],
				(char *)argv[4],
